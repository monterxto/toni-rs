...
pub fn handle_controller_consolidated(
    attr: TokenStream,
    item: TokenStream,
) -> Result<TokenStream> {
    ...
    if let Some(method_name) = init_method {
        let params = super::controller_struct::extract_constructor_params(&impl_block, &method_name)?;
        dependencies.init_method = Some(method_name.clone());
        dependencies.constructor_params = params;
        dependencies.source = crate::shared::dependency_info::DependencySource::Constructor(method_name);
        ...
        dependencies.constructor_params = params;
        dependencies.source = crate::shared::dependency_info::DependencySource::Constructor("new".to_string());
    }
